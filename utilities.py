...
def load_data(img_size=256) -> tuple:
    ...
    img_data = np.zeros((50, img_size, img_size, 3))  # TODO
    metadata = train_metadata.iloc[:50, 1:-1].values
    y = train_metadata.iloc[:50, -1].values
...
def pytorch_test_loop(dataloader, size, model, loss_fn, writer, epoch, device):
    test_loss = 0.0
